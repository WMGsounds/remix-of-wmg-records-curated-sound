    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-6 pb-24 pt-28 md:pt-32">
        ...
        <p className="mt-4 max-w-2xl text-sm leading-relaxed text-ivory/60">
          Permanent direct artwork URLs for Genius and other music platforms. Each URL uses the
          artist slug followed by the release slug — for example{" "}
          <span className="text-ivory/75">
            https://www.wmgsounds.com/api/media/release/betty-blane-heaven-in-your-arms.jpg
          </span>{" "}
          — and never expires. The image is generated on request from the Cover Art stored in
          Notion, so replacing the artwork in Notion updates the image at the same URL (within about
          an hour).
        </p>
        ...
        {!isLoading && !isError && (
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((release) => {
              ...
              const hasArtistSlug = Boolean(release.artistSlug);
              const url = hasArtistSlug ? mediaUrlFor(release.artistSlug, release.slug, origin) : "";
              return (
                <article
                  key={key}
                ...
                >
                  ...
                  {hasArtistSlug ? (
                    <>
                      ...
                      <div className="mt-4 flex flex-wrap items-center gap-2">
                        ...
                        >
                          ...
                          {copied === key ? "Copied" : "Copy Genius artwork URL"}
                        </button>
                        <a
                          href={url}
                          target="_blank"
                    ...
                    <p className="mt-3 rounded border border-ivory/10 bg-ink/60 px-3 py-2 text-[11px] leading-relaxed text-ivory/55">
                      No artist slug set in Notion for this release, so a permanent artwork URL can't
                      be generated yet.
                    </p>
                  )}
                </article>
